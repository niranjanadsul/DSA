...
class Solution {
    public static void main(String[] args) {
        ...
        int count = new Solution().ladderLength("hit","cog",list2);
        System.out.println(count+"=5");//5
        count = new Solution().ladderLength("a","c",list1);
        System.out.println(count+"=2");
        count = new Solution().ladderLength("hot","dog",list3);
    ...
    public int ladderLength(String beginWord, String endWord, List<String> wordList) {
        ...
        Map<String,List<String>> levelOrder = new TreeMap<>();

        int count=0;
        boolean isEndWord = false;
        while(!queue.isEmpty() && !isEndWord){
            ...
            for(int i=0;i<currQueueSize;i++){
                ...
                for(String pattern:patterns){
                    List<String> neighbours = matrix.getOrDefault(pattern, new ArrayList<>());
                    for(String neighbour:neighbours){
                        if(!visited.contains(neighbour)){
                            levelOrder.computeIfAbsent(word,x->new ArrayList<>()).add(neighbour);
                            queue.add(neighbour);
        ...
        dfs(beginWord,levelOrder,allPaths,"",endWord, count);
        return count;

    }

    public void dfs(String word, Map<String,List<String>> levelOrder,List<List<String>> allPaths,
                    String currPath, String endWord, int count){
        if(word.equals(endWord) && count==1){
            String path = currPath.substring(1)+","+word;
            allPaths.add(Arrays.stream(path.split(",")).toList());
        }else{
            for(String neighbour:levelOrder.getOrDefault(word,new ArrayList<>())){
                dfs(neighbour,levelOrder,allPaths,currPath+","+word,endWord, count-1);
            }
        }
    }


    public void buildMatrix(HashMap<String,List<String>> matrix, List<String> listNe){
        for(String word:listNe){
            List<String> patterns = getWordPatterns(word);
            for(String pattern:patterns){
                matrix.computeIfAbsent(pattern,x->new ArrayList<>()).add(word);
            }
        }
    }


    public List<String> getWordPatterns(String word){
        List<String> listNe = new ArrayList<>();
        for(int i=0;i<word.length();i++){
            StringBuilder pattern = new StringBuilder(word);
            pattern.setCharAt(i, '*');
            listNe.add(pattern.toString());
        }
        return listNe;
    }
