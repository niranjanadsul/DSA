...
public class OnlineStockSpan_13 {
    ...
    the price of the stock today is 2, then the span of today is 4 because starting from today,
    ...
    */
    ...
    public Stack<Integer[]> st;
            //price, span
    public OnlineStockSpan_13() {
        st=new Stack<>();
    }

    public int next(int price) {
        int currSpan = 1;//default span is 1
        while(!st.isEmpty() && st.peek()[0]<=price){
            currSpan+=st.pop()[1];
            //add the span of previous price to current span
            // if previous prices is <= current day price
        }
        st.push(new Integer[]{price,currSpan});
        return currSpan;
    }
}
